...
{
	for(int i=0;i<sizeof(srcmat[0]) / sizeof(srcmat);i++)
		for(int j=0;j<sizeof(srcmat[0]) / sizeof(srcmat);j++)
...
void rotate_pt(GLfloat x, GLfloat y, GLfloat theta)
{
	...
	for (int i=0; i<3; i++) //combines all points to newimage
	{
...
void display()
{
	...
	renderBitmapString(150,10,"Sahydri"); //Display the string at given position
